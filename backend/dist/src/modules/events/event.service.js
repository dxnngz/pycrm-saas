...
export class EventService {
    ...
    async createEvent(eventData, tenantId) {
    ...
    async updateEventById(id, userId, eventData) {
        ...
        if (eventData.start_date)
            updateData.start_date = new Date(eventData.start_date);
        if (eventData.end_date)
            updateData.end_date = new Date(eventData.end_date);
        return await prisma.event.update({
            where: { id },
            data: updateData
        });
    }
    async deleteEventById(id, userId) {
